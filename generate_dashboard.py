...
function renderCompare(){
  ...
  [['3','3 ans'],['5','5 ans'],['10','10 ans'],['max','Max']].forEach(function(b){
    var active=String(window._cmpEvoWindow)===b[0];
    html+='<button onclick="cmpSetEvoWindow(\''+b[0]+'\')" style="font-size:10px;padding:2px 10px;border-radius:100px;cursor:pointer;border:1px solid '+(active?'#DC2626':'var(--border)')+';background:'+(active?'#DC2626':'transparent')+';color:'+(active?'#fff':'var(--text3)')+';font-weight:'+(active?'600':'400')+'">'+b[1]+'</button>';
  });
  html+='</div>';
...
function cmpSetEvoWindow(win){
  ...
  document.querySelectorAll('[onclick^="cmpSetEvoWindow"]').forEach(function(btn){
    var bw=btn.getAttribute('onclick').match(/'([^']+)'/)[1];
