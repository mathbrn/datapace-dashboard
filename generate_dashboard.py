...
function updateBiggest(){
  ...
  var shortLabels=sorted.map(function(r){
    var s=r.r.replace(/^(TCS|BMW|Bank of America|Schneider Electric|NN|ASICS|HOKA|Adidas|Zurich|AJ Bell|Rock 'n' Roll Running Series)\s+/i,'');
    return s.length>28?s.substring(0,26)+'...':s;
  });
  var bigCfg={
    type:'bar',
    data:{labels:shortLabels,datasets:[{data:sorted.map(function(r){return(r.hist||{})[yr];}),backgroundColor:sorted.map(function(r){return colByName(r.r)+'CC';}),borderRadius:3,borderSkipped:false,barPercentage:0.7,categoryPercentage:0.85}]},
    options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,layout:{padding:{right:12}},
      plugins:{legend:{display:false},tooltip:{backgroundColor:'#1a1a1a',borderColor:'#333',borderWidth:1,titleColor:'#f0f0f0',bodyColor:'#ccc',padding:10,callbacks:{title:function(items){return sorted[items[0].dataIndex].r;},label:function(ctx){return' '+fmtFull(ctx.parsed.x)+' finishers';}}},
        datalabels:false},
      scales:{
        y:{grid:{display:false},ticks:{color:'#888',font:{size:11,weight:'400'},mirror:false,padding:8},border:{display:false}},
        x:{position:'top',grid:{color:'#ffffff06'},ticks:{color:'#555',font:{size:10},callback:function(v){return fmt(v);}},border:{color:'#ffffff08'}}
      }
    }
  };
